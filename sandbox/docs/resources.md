...
You have pre-fetched CSV data from the food helpline platform loaded into your sandbox. The data covers food pantries, soup kitchens, and SNAP/EBT resources.
...
| description | string | Free-text description of what the resource offers (may be empty) |
...
- Use `resources` as first point of reference — it has description and all key fields inline.
- To join with census/USDA/CDC data, use the crosswalk: `resources.merge(zip_county, on="zip_code").merge(census_poverty, on="fips")`.
