...
/** Census API: GET returns [ headers, ...rows ]. Keys from env. */
async function fetchCensus(
  ...
  const mapping = config.schemaMapping;
  const rows: CachedDatasetRow[] = dataRows.map((row) => {
...
/** BLS API v2: POST with series IDs. Key from env. */
async function fetchBls(config: PublicDatasetConfig): Promise<CachedDatasetRow[]> {
  ...
  // BLS API v2 returns Results.series (object with series array), not Results as array
  ...
  for (const s of results) {
    // LAUCN360610000000003 -> county_fips 36061
...
/** USDA ERS / Esri REST: query MapServer layer, return features[].attributes. */
